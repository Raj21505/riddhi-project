...
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "*,null,http://localhost:3000,http://localhost:8000,http://localhost:8001").split(",")
...
# Health endpoint while frontend is served at root
@app.get("/api/health")
async def health_check():
    return {"message": "Data Analysis API is running", "mode": "integrated"}
...
# Mount frontend at root so starting backend also serves UI.
# This is added after API routes so existing API endpoints keep priority.
